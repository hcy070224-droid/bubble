...
#三、考试安排表
    ...
    def generate_exam_table(self):#生成考试安排表
        ...
        if not os.path.exists("考场安排表"):
            os.mkdir("考场安排表")
        file_name = os.path.join("考场安排表", "考场安排表.txt")
        ...
        print("考场安排表生成")
        print("文件生成位置：", os.path.abspath(file_name))
...
#四、准考证
    def generate_admission_cards(self, shuffled):
        ...
        print("准考证生成")
        print("准考证目录：", os.path.abspath(folder))
#验证是否输入为数字
    @staticmethod
...
if __name__ == "__main__":#确保这段代码只在直接运行这个文件时执行
    ...
    while True:#选择功能 #while true 保证程序持续运行，直到用户主动退出
        ...
        if choice == '1':
            ...
            if 'shuffled' not in locals():
                print("请先生成考场安排表")
            else:
                system.generate_admission_cards(shuffled)#如果用户没先执行第3步→程序会报错
        elif choice == '0':
            break
        else:
            print("无效选择") #排除其他异常
